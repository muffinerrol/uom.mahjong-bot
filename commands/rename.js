...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('rename')
        .setDescription('Change your name on the monthly leaderboard.')
    ...
    async execute(interaction) {
        ...
        const searchResult = await sheetCommands.searchNameScoreless(searchTerm);
        ...
        for (foundName in searchResult) {
            rowOptions.push({ label: searchResult[foundName][0].toString(), value: searchResult[foundName][0].toString() });
        };
  
        const searchResultRow = new ActionRowBuilder()
            .addComponents(
                new SelectMenuBuilder()
                    .setCustomId('selectRename')
